...
def main():
    ...
    for t in range(cfg.total_steps):
        key, action_key, sample_key, update_key, reset_key, eval_key = jax.random.split(key, 6)
        ...
        done_bool = bool(jax.device_get(done))
        timeout_bool = episode_length >= cfg.max_episode_steps

        if done_bool or timeout_bool:
            episode_count += 1
            print(
                f"[episode {episode_count}] "
                f"step={t + 1}, return={episode_return:.2f}, length={episode_length}"
            )
    ...
    print("Training smoke run finished.")
